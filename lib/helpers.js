...
const parseResults = (editor, results, filePath, showIgnored) => {
  startMeasure('linter-stylelint-plus: Parsing results');
  if (!results) {
    endMeasure('linter-stylelint-plus: Parsing results');
    endMeasure('linter-stylelint-plus: Lint');
    return [];
  }
  ...
  const warnings = results.warnings.map((warning) => {
    ...
    if (ruleParts.length === 1) {
      ...
      const linterStylelintURL = 'https://github.com/AtomLinter/linter-stylelint-plus/tree/master/docs';
      switch (pluginName) {
        case 'plugin':
...
export const runStylelint = async (editor, stylelintOptions, filePath, settings) => {
  startMeasure('linter-stylelint-plus: Stylelint');
  let data;
  ...
  try {
    ...
    endMeasure('linter-stylelint-plus: Stylelint');
...
export function getDefaultConfig(syntax) {
  ...
  if (!packagePath) {
    packagePath = atom.packages.resolvePackagePath('linter-stylelint-plus');
  }
